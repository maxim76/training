"""
Another class-oriented variant. 
"""
...
class TCPPacket( AbstractL4Packet ):
	...
	def packHeader( self ):	# input param are for checksum
		...
		tcpChecksum = checkSum( pseudoHdr + str( self.payload ) )
...
class UDPPacket( AbstractL4Packet ):
	...
	def packHeader( self ):
		...
		udpChecksum = checkSum( pseudoHdr + str( self.payload ) )
...
class ICMPPacket( AbstractL4Packet ):
	...
	def packHeader( self ):
		...
		icmpChecksum = checkSum( pseudoHdr + str( self.payloadICMP ) )
...
"""
Wrappers that run net functions with root privileges
"""

snippetInjectUDP="""
from helpers.net_tools import UDPPacket
UDPPacket( "%s", "%s", bytearray.fromhex( "%s" ) ).inject()
"""
def injectUDP( srcAddress, dstAddress, data ):
	...
	return runAsRoot( snippetInjectUDP % ( srcAddress, dstAddress, data ) )

snippetInjectTCP="""
from helpers.net_tools import TCPPacket
TCPPacket( "%s", "%s", bytearray.fromhex( "%s" ) ).inject()
"""
def injectTCP( srcAddress, dstAddress, data ):
	...
	return runAsRoot( snippetInjectTCP % ( srcAddress, dstAddress, data ) )

snippetInjectICMP="""
from helpers.net_tools import ICMPPacket
ICMPPacket( "%s", "%s", %d, %d, bytearray.fromhex( "%s" ) ).inject()
"""
def injectICMP( srcAddress, dstAddress, icmpType, icmpCode, data ):
	...
	return runAsRoot( snippetInjectICMP % ( srcAddress, dstAddress, icmpType, \
		icmpCode, data ) )
